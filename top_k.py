...
class TorchTextEmbedder:
    """
    既存方針踏襲：
    - sentence-transformers があれば自動使用（--model_name で明示可）
    - 無ければハッシュ平均埋め込み（再現性あり、dim=768）
    """
    def __init__(self, model_name: Optional[str] = None, device: Optional[str] = None, dim: int = 768):
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.dim = dim
        self._st_model = None

        self._try_setup_sentence_transformers(model_name)

        if self._st_model is None:
            g = torch.Generator().manual_seed(42)
            self._emb_vocab = 65536
            self._emb = torch.nn.Embedding(self._emb_vocab, self.dim)
            torch.nn.init.normal_(self._emb.weight, mean=0.0, std=0.02, generator=g)
            self._emb.to(self.device)
            self._emb.eval()

    def _try_setup_sentence_transformers(self, model_name: Optional[str]):
        try:
        ...
        try:
            ...
            self._st_model = SentenceTransformer(name, device=self.device)
            self._st_model.max_seq_length = getattr(self._st_model, "max_seq_length", 512)
        except Exception as e:
    ...
    def encode(self, texts: List[str], batch_size: int = 32) -> np.ndarray:
        ...
        if self._st_model is not None:
            try:
                ...
                print(f"⚠️ STエンコード失敗: {e}. フォールバック使用")
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for s in sub:
                arr = np.frombuffer(s.encode("utf-8", errors="ignore"), dtype=np.uint8)
                h = (arr.astype(np.uint32) * 1315423911) & 0xFFFFFFFF
                h = torch.from_numpy(h % self._emb_vocab).long().to(self.device)
                if h.numel() == 0:
                    h = torch.zeros(1, dtype=torch.long, device=self.device)
                idxs.append(h)
            batch_vecs = []
            for ids in idxs:
                e = self._emb(ids)               # (L, dim)
                v = e.mean(dim=0, keepdim=True)   # (1, dim)
                batch_vecs.append(v)
            out = torch.cat(batch_vecs, dim=0)    # (b, dim)
            out_list.append(out.detach().cpu())
        return torch.cat(out_list, dim=0).numpy().astype(np.float32)
...
class Config():
    ...
    output_folder_a = f'/mnt/eightthdd/graph/csv{CSV}/{REF_CONT}/{output_folder_name}{CSV}_a'  # 出力フォルダのパスを指定
    output_folder_b = f'/mnt/eightthdd/graph/csv{CSV}/{REF_CONT}/{output_folder_name}{CSV}_b'  # 出力フォルダのパスを指定

    REF_CONT = "cont"
    output_folder_cont = f'/mnt/eightthdd/graph/csv{CSV}/{REF_CONT}/{output_folder_name}{CSV}_b'  # 出力フォルダのパスを指定
    ...
    TEXT_A_DIR = f"/mnt/eightthdd/graph/csv{CSV}/{REF_CONT}/{input_folder_name}{CSV}_a"
    TEXT_B_DIR = f"/mnt/eightthdd/graph/csv{CSV}/{REF_CONT}/{input_folder_name}{CSV}_b"
    ...
    corpus_folder = f"/mnt/eightthdd/graph/csv{CSV}/corpus"
    CORPUS_ROOT = Path(corpus_folder).resolve()
    CORPUS_ROOT.mkdir(parents=True, exist_ok=True)

    query_folder = f"/mnt/eightthdd/graph/csv{CSV}/query"
    QUERY_ROOT = Path(query_folder).resolve()

    out_csv = f"/mnt/eightthdd/graph/csv{CSV}/results/topk_results_{CSV}.csv"
...
def configure_faiss():
    ...
    df = pd.read_csv("/mnt/eightthdd/graph/csv1/ref/df_concat1.csv")
    df_cont = pd.read_csv("/mnt/eightthdd/graph/csv1/cont/df_concat1.csv")
    # dfのsyutugan列を取得
    ...
    #     if i // CFG.files_per_part == 0:
    #         p = CFG.OUTPUT_ROOT_B / f"{i}.txt"
    #         if not p.exists():
    #             continue
    #         target_file_name = ref_column[i]
    #
    #     if i // CFG.files_per_part == 1:
    #         p = CFG.OUTPUT_ROOT_CONT / f"{i}.txt"
    #         if not p.exists():
    #             continue
    #         target_file_name = ref_cont_column[i]
    #
    #     if i // CFG.files_per_part == 2:
...
if __name__ == "__main__":
    # 最小動作サンプル（手元の data/corpus / data/query に合わせて書き換え）
    # 実運用ではコマンドライン引数で呼んでください。
    # 例:
    # python faiss_topk_search.py --corpus_dir ./data/corpus --query ./data/query/1.txt --topk 5
    INIT = False
    if INIT:
        configure_faiss()
    import sys
    sys.argv = [
        ...
        # "--query", "/mnt/eightthdd/graph/csv1/ref/g_text1_a/0.txt",
